...
interface IGovernor {
  ...
  function getRoleName(
    uint8 role
  ) external view returns (string memory roleName);
  function getRolesForUser(
    address user
  ) external view returns (uint8[] memory rolesForUser);
  function getRolesFromByteMap(
    bytes32 byteMap
  ) external pure returns (uint8[] memory roleIds);
  function getRolesWithCapability(
    address,
    bytes4
  ...
  function getUsersByRole(
    uint8 role
  ) external view returns (address[] memory usersWithRole);
  function isPublicCapability(
    address target,
    bytes4 functionSig
  ) external view returns (bool);
