...
class ApeSafe(Safe):
    ...
    def multisend_from_receipts(
        self, receipts: List[TransactionReceipt] = None, safe_nonce: int = None
    ) -> SafeTx:
        """
        ...
        txs = [
            MultiSendTx(MultiSendOperation.CALL, tx.receiver, tx.value, tx.input)
            for tx in receipts
        ]
    ...
    def get_signer(
        self, signer: Optional[Union[LocalAccount, str]] = None
    ) -> LocalAccount:
        if signer is None:
        ...
        assert isinstance(
            signer, LocalAccount
        ), "Signer must be a name of brownie account or LocalAccount"
        return signer
    ...
    def sign_with_trezor(
        ...
        client = TrezorClient(
            transport=transport, ui=ui.ClickUI(passphrase_on_host=not use_passphrase)
        )
        ...
        if force_eth_sign:
            ...
            use_eip712 = (
                ...
                client.features.minor_version,
                client.features.patch_version,
            ) >= (
                2,
                4,
                3,
        ...
        if use_eip712:
            trez_sig = ethereum.sign_typed_data(
                client, path, safe_tx.eip712_structured_data
            )
            ...
            trez_sig = client.call(
                EthereumSignMessage(address_n=path, message=safe_tx.safe_tx_hash)
            )
            v, r, s = signature_split(trez_sig.signature)
    ...
    def sign_with_frame(
        self, safe_tx: SafeTx, frame_rpc="http://127.0.0.1:1248"
    ) -> bytes:
        """
        ...
        signature = frame.manager.request_blocking(
            "eth_signTypedData_v4", [account, safe_tx.eip712_structured_data]
        )
    ...
    def preview(self, safe_tx: SafeTx, events=True, call_trace=False, reset=True):
        ...
        receipt = owners[0].transfer(
            payload["to"],
            payload["value"],
            gas_limit=payload["gas"],
            data=payload["data"],
        )
    ...
    def execute_transaction_with_frame(
        self, safe_tx: SafeTx, frame_rpc="http://127.0.0.1:1248"
    ) -> bytes:
        """
